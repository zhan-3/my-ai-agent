...
四幕演示（全部作用于真实主管图，session.chat 默认 = 图工厂调度图）：
  第1幕 动态发现（渐进式披露）：注册表扫描内置六子 Agent + 外部扩展，
        只读 AST 元数据，不执行任何子 Agent 代码
  第2幕 懒加载：未派发的子 Agent 不加载（sys.modules 为证）；派发才加载（哨兵为证）
  第3幕 热插拔：运行中向 plugins/ 新增子 Agent → 文件层即时感知 → 图工厂指纹缓存
        自动重建（下一次会话即认识新意图，主管零改动、无 reload 副作用）
  第4幕 外部扩展真实路由：差旅统计（第七意图）在真实主管图里被识别并派发
"""
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("第3幕｜热插拔：运行中新增子 Agent → 文件层即时感知 → 图工厂自动重建")
...
'''
    ...
    # 主管零改动：图工厂指纹缓存——新 manifest 指纹与缓存不同 → 下一次会话自动重建新图
    ...
    print("=" * 60)
    print("第4幕｜外部扩展真实路由：差旅统计在真实主管图里被识别并派发")
    q2 = "统计一下我的出差情况"
    r2 = chat(q2)
    print(f"  用户：{q2}")
    ...
    # 清理热插拔演示文件（恢复基线：内置六 + 外部 stats）——下次会话指纹自动回退，无需 reload
