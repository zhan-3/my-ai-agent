...
约 28 个用例，真实 LLM；不进确定性门禁，对话/意图/编排行为变化后按需回归。

覆盖范围：纯要素列举识别、修改/改期/取消已有行程、返程早于出发、模糊日期、性别/人数分房、
双隔离（账号/线程）、偏好幂等与覆盖、政策 QA（报销时限/订票提前量/热线/FAQ/城市提示/绿色倡议/
应急/标准审批）、联网查询 grounded 断言（当地时间/扩展天气字段）、境外预算与时差提示、票务门禁排除。
