...
迁移前主管执行一次 `classify → route → execute → merge` 后结束。它可以拆分多意图并保存对话状态，
...
6. 政策证据、天气失败和写回前验证由确定性策略门执行；主管文本不能覆盖策略门结果。
...
子 Agent 和领域模块无需重写。主管控制流已从预定义 DAG 切换为模型驱动 Loop；可观察轨迹、预算、
取消语义和确定性策略门由 Loop 与会话模块统一承载。旧图工厂和对应拓扑测试已删除，不维护双运行时。
