...
1. ✅ **21 条合理素材并入黄金集**（56 → 77 条），基线 99% 无回退。
2. ✅ **轻量消歧**（路线图第四阶段修订版，曾否决多模型投票）：只对歧义子集反问，多轮实现（不加图回边）；纯规则启发式。落地：`disambiguation.py` + 图门 `clarify_gate`（短路 END）。
3. ✅ 其他候选：
   - 行程 `from_city`「无→北京」一致性：哨兵归一化（`无/未知/出差`→`待定`）
   - 更多 E2E 场景矩阵（E2E-05~08；顺带修多轮要素延续：提取器注入 recent 上下文）
   - SSE error 路径覆盖（流中途炸/空产出/端点未消化异常）
   - **航班/车次能力本身 → 已决：现阶段不实现**。原因：需票务接口，最终设计是直接购票；初学阶段难以实现。产品行为已对齐——消歧选①时确定性直答「暂不支持实时航班/车次查询」（不静默当北京编造）。
