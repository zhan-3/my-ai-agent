...
| 测试框架 | pytest 9 + pytest.mark | 分层测试 117 个：单元 102（无 LLM，含 6 个 Postgres 条件跑）+ 集成 15（真实模型） |
...
| E 评测与测试 | ✅ 完整 | 分层自动化测试 117 个：单元层 102（记忆/行程/注册中心/稳定性/RAG 分块/图工厂/会话隔离/认证/webapp 端点，无 LLM；其中 6 个 Postgres 真库测试需本地容器+`POSTGRES_TEST_URL` 才跑）+ 集成层 15（意图识别 7 用例含边界 + 外部扩展识别 2 + 多意图拆分 2、端到端记忆闭环 + 外部扩展端到端派发 + 多意图并行、向量 RAG 检索）；`uv run pytest` / `-m integration` |
